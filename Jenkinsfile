        ...
        stage('Precheck') {
            steps {
                ...
                script {
                    ...
                    // duplicated code, only the other un-refined stages
...
def deleteClient() {
    def deletionMessage = shCmd(aCurl() + """-X DELETE "https://apigw.${NOM.NOM_BASE_DOMAIN}/auth/api/v1/apps/registrations/${CLIENT_ID}" \
            -u "${NOM.NOM_USER_NAME}:${NOM.NOM_PASSWORD}"
        """, "Delete client with client id ${CLIENT_ID}")
