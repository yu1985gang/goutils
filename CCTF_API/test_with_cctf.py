...
class CCTFProject(requests.Session):
    ...
    def execute_tagged_testcases_in_project(self, requested_tag=""):
        self.tc_result_list = []
        for pkg_id in self.__package_id_list:
            self.__package_id = pkg_id
    ...
    def __find_testcases_by_tag(self, requested_tag_string):
        if requested_tag_string == "":
            testcases = self.__get_all_testcases()
        else:
            testcases = []
            orginal_tag_list = str(requested_tag_string).split(",")
            for tag in orginal_tag_list:
                if tag in TEST_GROUP_LIST:
                    testcases += self.__get_group_testcases(tag)
                else:
                    testcases += self.__get_tagged_testcases(tag)

        tagList = []
        for test_case in testcases:
            tagList.append(str(test_case["testId"]))
    ...
    def get_execution_logs_by_jobid(self, filename):
        url = self.base_url + "/job/test/log/?jobId=" + str(self.__job_id)
        if self.__is_target_downloadable(url):
            resp = self.get(url, verify=False, allow_redirects=True)
            self.__check_response(resp)
            # filename = "testlog.zip"
            open(filename, 'wb').write(resp.content)
        else:
            LOGGER.error("Target [%s] is not downloadable.", url)
    ...
    def update_package_parameters(self, parameters_in_json):
        final_parameters = {
...
def generate_test_parameters(parameter_string):
    ...
    for para_pair in parameter_list:
        ...
        if name.startswith("TP_") or name.startswith("tp_"):
            ...
            ne_parameters.update({name[3:]:value})
...
if __name__ == '__main__':
    test = CCTFProject("neo0033vip.netact.nsn-rdnet.net", "admin", "cctf@Pipe", "TAG_Test", "DEBUG")
    test.create_lab("remote", "https://repo.lab.pl.alcatel-lucent.com/neo-generic-candidates/CCTF/neo-sat-poc-0.0.175.tgz", "robot1", "neo-docker-candidates.repo.lab.pl.alcatel-lucent.com/neo-sat-poc:latest")
    # test.create_lab("remote", "https://repo.lab.pl.alcatel-lucent.com/neo-generic-releases/CCTF/neo-sat-poc-0.0.20.tgz", "robot2", "")
    test.update_package_parameters(generate_test_parameters("TP_BASE_DOMAIN=neo0033.dyn.nesc.nokia.net;TP_NOM_OPEN_API_GW=apigw.neo0033.dyn.nesc.nokia.net;TP_NOM_OPEN_API_USERNAME=neonetics;TP_NOM_OPEN_API_PASSWORD=Nextgen#123;NE_NE_ID=739;NE_NE_TYPE=MRBTS;NE_INTEGRATED_ID=MRBTS-1813;NE_FM_SPECIFIC_PROBLEM=7220;NE_METADATA_FASTPASS_PACKAGE_LINK=https://esisoj70.emea.nsn-net.net/artifactory/netact-fast-pass-release-local/fast_pass_packages/SBTS20C_ENB_9999_200602_000006/20200615T052532/SBTS20C_ENB_9999_200602_000006_20200615T052532.zip"))
    test.execute_tagged_testcases_in_project("ID_3003,ID_3004")
    test.get_execution_logs_by_jobid()
