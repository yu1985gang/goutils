...
def getFPLink(ne_sw_id) {
    // TODO Get FP_PACKAGE_LINK from NE_SW_ID
    // hard code temporarily
    return "https://esisoj70.emea.nsn-net.net/artifactory/netact-fast-pass-release-local/fast_pass_packages/SBTS20C_ENB_9999_200602_000006/20200615T052532/SBTS20C_ENB_9999_200602_000006_20200615T052532.zip"
}

def getDefaultConf() {
    def Conf = readYaml(file: "${env.WORKSPACE}/configuration/syve.yaml")
    if(Conf.NOM.size() < 1) {
...
def pingAddress(String address, int account = 3) {
    if (Utils.isIPv6(address)){
        def rc = sh script: "ping6 -c ${account} ${address}", returnStatus: true, label: "Ping ipv6 address"
    }else{
